...
import Page2.JoinPage.BackActionListener;
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (login_id.equals("") && login_pass.equals("")) {
				...
				try {
					 ...
					 * stmt.executeQuery(sql); //결과를 담을 ResultSet 생성 후 결과 담기
					 * 
					 * //결과를 담을 ArrayList생성 ArrayList<User> userlist = new ArrayList<User>();
					 * 
					 * //ResultSet에 담긴 결과를 ArrayList에 담기 while(rs.next()) { User user = new User();
					 * user.setName(rs.getString("name")); user.setId(rs.getString("id"));
					 * user.setPass(rs.getString("pass"));
					 * user.setRegular_chk(rs.getString("regular_chk")); userlist.add(user); }
					 * 
					 * //결과물 출력 for(int i=0; i<userlist.size(); i++) {
